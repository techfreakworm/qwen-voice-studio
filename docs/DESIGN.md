...
| Load (as shipped) | `from_pretrained(device_map="mps", dtype=bf16, attn="sdpa")` | CPU load **resident in parent** (background warmup) → `.to("cuda")` per `@spaces.GPU` fork (§13, D9) |
| attn / compile | sdpa / no torch.compile | sdpa / no torch.compile (AOT-inductor = post-v1 only) |
| Residency | all-3-resident; request-time adaptive evict >60 GB (M2, darwin-only) | resident in parent (background warmup) + fork_move CPU→GPU per request (D9) |
...
Empirically (D9): models are **warmed resident in the parent process** (a background thread at startup, `fork_move` residency) and moved CPU→GPU inside each `@spaces.GPU` fork — a fork that *loads* the model loses it on exit, so lazy-load-in-fork reloaded ~4.5 GB every request (~38 s, GPU idle most of it); loading at module level *before* `launch()` overran the startup window (RUNTIME_ERROR). With resident models each request is a fast CPU→GPU move + generate (~6 s, `cuda:0` confirmed). All state-carrying features are stateless per-request (D8). Handlers are `@spaces.GPU`-decorated (generation, save-voice, design-bridge, lora quick-test); state/inspect handlers stay undecorated (parent). Quota: PRO 40 min/day included; **overage = pre-paid credits = operator approval required** (verified staying within the included minutes — no credits used).
...
Verified deltas from the pre-build design: ZeroGPU models are **warmed resident in the parent** (background thread) and moved CPU→GPU per request (`fork_move`) — lazy-load-in-fork reloaded ~4.5 GB/request (~38 s); module-level preload before `launch()` overran startup (**D9**); Gradio launched with `server_name="0.0.0.0"`, `ssr_mode=False`; RAM watchdog disabled on ZeroGPU (container psutil misreports); deprecation warnings filtered; `packages.txt` = sox/ffmpeg; binaries via git-LFS; **LoRA per-request (D8)**. Space verified end-to-end (5/5 modes, real audio, `cuda:0`, ~6 s/request) within PRO's included 40 min/day ZeroGPU quota — **no paid credits used**.
